...
def test_onboarding_send_link(invoke, mock_api):
    mock_api.post(f"{_BASE}/accounts/org-onboard-1/send-link").respond(200, json={
...
def test_onboarding_impersonate(invoke, mock_api):
    mock_api.post(f"{_BASE}/accounts/org-onboard-1/impersonate").respond(200, json={
...
def test_onboarding_activate_no_reset(invoke, mock_api):
    ...
    result = invoke([
        "admin", "onboarding", "activate", "org-onboard-1", "--no-send-password-reset",
    ])
    assert result.exit_code == 0


def test_onboarding_deactivate(invoke, mock_api):
    mock_api.post(f"{_BASE}/accounts/org-onboard-1/deactivate").respond(200, json={
        "status": "pending",
    })
...
def test_onboarding_update_config(invoke, mock_api):
    mock_api.patch(f"{_BASE}/accounts/org-onboard-1/config").respond(200, json={
        "show_calendly": True,
        "calendly_url": "https://calendly.com/updated",
    })
    result = invoke([
        "admin", "onboarding", "update-config", "org-onboard-1",
        "--calendly-url", "https://calendly.com/updated",
    ])
    assert result.exit_code == 0
...
def test_onboarding_update_settings(invoke, mock_api):
    ...
    result = invoke([
        "admin", "onboarding", "update-settings",
        "--calendly-url", "https://calendly.com/new",
    ])
    assert result.exit_code == 0
    assert "Updated" in result.output
