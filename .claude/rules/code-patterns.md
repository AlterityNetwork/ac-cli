...
## API Route Prefix
- All CRM routes use the `_CRM` constant from `crm/__init__.py` (currently `/api/v1/crm`)
...
- Non-CRM commands (auth, health) handle errors directly in the command function
...
- `--json` is a global option on the crm app group callback, passed via `typer.Context`
